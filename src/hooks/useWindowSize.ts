...
export function useWindowSize(targetWindow: Window | undefined) {
  if (!targetWindow) {
    console.warn(
      "useWindowSize: targetWindow is undefined, using default window."
    );
    targetWindow = window;
  }
...
export function useMoveImageOnResize() {
  ...
  const [size, setSize] = useState(() => {
    if (!currentWindow) return { width: 0, height: 0 };
    return {
