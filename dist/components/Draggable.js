...
export const ImageAtIndexFullScreenMemo = memo((props) => {
    ...
    const handleZoomToPoint = useCallback((clickX, clickY) => {
        ...
        const imageRelativeX = clickX - imageRect.left + scrollContainer.scrollLeft;
        const imageRelativeY = clickY - imageRect.top + scrollContainer.scrollTop;
        console.log("Image-relative click at:", imageRelativeX, imageRelativeY);
        console.log("As fraction of image:", imageRelativeX / imageRect.width, imageRelativeY / imageRect.height);
        // Now zoom
        ...
        console.log("New image-relative position:", newImageRelativeX, newImageRelativeY);
        // Scroll to keep that point centered
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                const containerWidth = scrollContainer.clientWidth;
                const containerHeight = scrollContainer.clientHeight;
                scrollRef.current?.scrollTo(newImageRelativeX - containerWidth / 2, newImageRelativeY - containerHeight / 2, "auto");
            });
        });
