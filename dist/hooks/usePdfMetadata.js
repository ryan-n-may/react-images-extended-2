...
export function usePdfMetadata() {
    ...
    useEffect(() => {
        ...
        if (sourceType !== ILightboxImageType.PDF) {
            console.warn("usePdfMetadata: sourceType is not PDF, skipping metadata extraction.");
            return;
        }
        if (!pdfDocumentSrc) {
            console.warn("usePdfMetadata: pdfDocumentSrc is undefined, skipping metadata extraction.");
            return;
        }
